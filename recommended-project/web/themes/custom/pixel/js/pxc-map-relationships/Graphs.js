...
class Graph{
  ...
  activate_building(){
    ...
    function release_click(e){
      console.log("release" + " " + e.which + " " + this.active.text  + " " + this.active.tid);
      bolita1();
      if(e.which == 2){
        return "";
      }
    ...
    function click_down(e){
      console.log("down" + " " + e.which + " " + this.active.text  + " " + this.active.tid);
      if(e.which == 2){
        return "";
      }
      if(e.which == 1){
  ...
  node(x=false, y=false, r=false, text="", tid=""){
    let node = new Graph._node(this.id, x=x, y=y, r=r, text=text, tid=tid);
...
Graph._edge = function(contextid, startNodeid, endNodeid, color="#000", text="", directional=false, tid=""){
    this.type = "edge";
...
Graph._node = function(contextid, x=false, y=false, r=false, text="", tid=""){
    this.type = "node"
    this.children = [];
    this.edges = {};
    this.contextid = contextid;
    ...
    this.create = function(x,y,r,text,tid){
        ...
        if(this.tid in Graph.functions){
          this.func = Graph.functions[this.tid];
        }
    ...
    this.build = function(){
      ...
      context.ctx.fillText(this.tid, this.x,this.y);
